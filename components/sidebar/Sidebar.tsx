...
export default function Sidebar({ className }: { className: string }) {
  const { workspaceItems } = useQuery(
    api.workspaceFunctions.workspaceItems,
  ) ?? { workspaceItems: [] };
  ...
  return (
    <>
      <div
        className={cn(
          "text-white h-full sidebar bg-[var(--header-background)] overflow-y-auto border-r-1 border-header-border",
          className,
        )}
      >
        <DialogCloseButton />
        {workspaceItems.map((workspaceItem) => (
          ...
          >
            <span title={workspaceItem.workspacename} className="text-sm">
              {workspaceItem.workspacename}
            </span>
            {workspaceItem.status === "private" && (
              <div title="Private">
                <Lock />
              </div>
            )}
            {workspaceItem.status === "public" && (
              <div title="Public">
                <Globe />
              </div>
            )}
